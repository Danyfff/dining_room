...
class BaseWorker:
    ...
    def check_user_login(self, user: "User") -> "Login":
        ...
        return Login(id=resp.json()['id'], user_post=resp.json()['post_id'])

    def get_users_list(self):
        resp = requests.get(url=self.end_points.get('users'))
        return [User(id=user['id'], login=user['login'], reg_date=user['reg_date'], post=user['post'])
...
class Login:
    ...
    def __init__(self, phone: str, post: str = None, id: int = None, reg_date: str = None, post_id: str = None, password: str = None):
        self.user_id = id
        self.login = phone
        if reg_date:
    ...
    def to_json(self):
        return f'{{"phone": "{self.login}", "password": "{self.password}", "post_id": {self.post_id}}}'
